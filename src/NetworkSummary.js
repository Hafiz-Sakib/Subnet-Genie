...
function summarizeNetworks(networks) {
  ...
  if (networks.length === 1) {
    ...
    return {
      supernet: n.cidr,
      prefix: n.prefix,
      network: intToIP(n.network),
      broadcast: intToIP(n.broadcast),
      networks,
    };
  }
  ...
  return {
    ...
    networks,
    efficiency: (
      (networks.reduce((a, n) => a + Math.pow(2, 32 - n.prefix), 0) /
        Math.pow(2, 32 - prefixLen)) *
      100
    ).toFixed(1),
  };
}
...
export default function NetworkSummary() {
  ...
  const addRow = () => {
    if (inputs.length < 20) setInputs([...inputs, ""]);
  };
  const removeRow = (i) => {
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          height: 500,
          background:
            "radial-gradient(circle, rgba(116,185,255,0.08) 0%, transparent 70%)",
          top: -100,
          right: -100,
        }}
      />
      ...
      >
        <div className="animate-fadeInUp" style={{ marginBottom: 36 }}>
          ...
          <p style={{ fontSize: 13, color: "var(--text-secondary)" }}>
            Summarize multiple IP networks into a single supernet (route
                  ...
                  {result.networks.map((n, i) => (
                    <tr key={i}>
                      ...
                      <td>
                        ...
                        >
                          {(Math.pow(2, 32 - n.prefix) - 2).toLocaleString()}
                        </span>
                      </td>
                    </tr>
                  ))}
