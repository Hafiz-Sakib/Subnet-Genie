  ...
  return (
    <nav className="nav-bar">
      ...
      <div className="nav-links">
        ...
        <Link to="/normal-subnet" className="nav-link">FLSM</Link>
...
const VLSMResults = () => {
  ...
  const copyAll = () => {
    const text = subnetResults.map((r, i) =>
  ...
  const exportCSV = () => {
    ...
    const rows = subnetResults.map(r =>
    ...
    const a = document.createElement("a"); a.href = url; a.download = "vlsm_results.csv"; a.click();
    setToast("CSV exported!");
  };

  const totalHosts = subnetResults.reduce((a, r) => a + r.requiredHosts, 0);
  const maxSubnetSize = subnetResults.length > 0 ? Math.max(...subnetResults.map(r => Math.pow(2, 32 - parseInt(r.subnetMaskCIDR)))) : 1;

  const palette = ['#fabd2f','#06d6a0','#74b9ff','#fd79a8','#a29bfe','#fdcb6e','#55efc4','#fd9644','#e17055','#00b894'];

  return (
    <div className="page-wrapper" style={{ background: 'var(--bg-deep)', minHeight: '100vh' }}>
      <div className="bg-grid" />
      <div className="bg-glow-orb" style={{ width: 400, height: 400, background: 'radial-gradient(circle, rgba(6,214,160,0.07) 0%, transparent 70%)', top: 0, left: '30%' }} />
      <NavBar />
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '48px 24px 60px' }}>
        ...
        <div className="animate-fadeInUp" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginBottom: 32 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 4 }}>{subnetResults.length} subnet{subnetResults.length !== 1 ? 's' : ''} allocated</p>
          </div>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={{ display: 'flex', gap: 4, background: 'rgba(255,255,255,0.04)', borderRadius: 8, padding: 4, border: '1px solid var(--border-subtle)' }}>
              {['table', 'visual'].map(m => (
                <button key={m} onClick={() => setViewMode(m)} style={{ padding: '6px 14px', borderRadius: 6, border: 'none', cursor: 'pointer', fontFamily: 'Syne, sans-serif', fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', background: viewMode === m ? 'var(--cyan)' : 'transparent', color: viewMode === m ? '#0a0c10' : 'var(--text-muted)', transition: 'all 0.2s' }}>
                  {m}
                </button>
              ))}
            </div>
            ...
            {viewMode === 'table' ? (
                    ...
                    <tbody>
                      {subnetResults.map((result, index) => (
                        <tr key={result.networkAddress}>
                          <td>
                            <span className="badge badge-cyan">{result.requiredHosts}</span>
                          </td>
                          <td>
                            <span className="ip-display">{result.networkAddress}/{result.subnetMaskCIDR}</span>
                          </td>
                          ...
                          <td>
                            <button className="copy-btn" onClick={() => copyToClipboard(`${result.networkAddress}/${result.subnetMaskCIDR}`)}>⎘</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              <div className="card" style={{ padding: 28 }}>
                <div style={{ marginBottom: 20 }}>
                  <span style={{ fontFamily: 'Syne, sans-serif', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Address Space — Proportional Allocation</span>
                </div>
                ...
                <div style={{ display: 'flex', height: 32, borderRadius: 6, overflow: 'hidden', marginBottom: 24, gap: 2 }}>
                  {subnetResults.map((r, i) => {
                    ...
                    const width = (size / maxSubnetSize) * 100;
                    return (
                      <div key={i} title={`${r.networkAddress}/${r.subnetMaskCIDR} — ${r.requiredHosts} hosts`}
                        style={{ flex: size, background: palette[i % palette.length], opacity: 0.8, minWidth: 4, transition: 'opacity 0.2s', cursor: 'pointer' }}
                      ...
                      <div key={i}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <span className="ip-display" style={{ fontSize: 13 }}>{r.networkAddress}/{r.subnetMaskCIDR}</span>
                          </div>
                          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                            <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 11, color: 'var(--text-muted)' }}>{r.requiredHosts} / {usable} hosts</span>
                            <span className="badge" style={{ background: efficiency > 70 ? 'rgba(6,214,160,0.1)' : 'rgba(250,189,47,0.1)', color: efficiency > 70 ? 'var(--cyan)' : 'var(--gold)', border: `1px solid ${efficiency > 70 ? 'rgba(6,214,160,0.2)' : 'rgba(250,189,47,0.2)'}`, fontSize: 10 }}>{efficiency}% eff.</span>
                          </div>
                        </div>
                        <div className="subnet-bar">
                          <div className="subnet-bar-fill" style={{ width: `${efficiency}%`, background: `linear-gradient(90deg, ${palette[i % palette.length]}cc, ${palette[i % palette.length]}44)` }} />
                        </div>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 28, flexWrap: 'wrap' }}>
          <button className="btn-primary" style={{ background: 'var(--cyan)', boxShadow: 'none' }} onClick={() => navigate("/vlsm-subnet")}
            onMouseEnter={e => { e.currentTarget.style.boxShadow = '0 8px 24px rgba(6,214,160,0.3)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
          ...
          <Link to="/normal-subnet" className="btn-secondary" style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}>Try FLSM →</Link>
        </div>
      </div>
