  ...
  return (
    <nav className="nav-bar">
      ...
      <div className="nav-links">
        ...
        <Link to="/vlsm-subnet" className={`nav-link ${location.pathname === "/vlsm-subnet" ? "active" : ""}`}>VLSM</Link>
      </div>
    </nav>
  );
}
        ...
        <div className={history.length > 0 ? 'form-history-grid' : ''}>
          ...
          <div className="card animate-fadeInUp stagger-1" style={{ padding: 32 }}>
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
              ...
              {numSubnetsInt > 0 && (
                <div>
                  <label className="field-label" style={{ marginBottom: 12 }}>Required Hosts per Subnet</label>
                    ...
                    {Array.from({ length: numSubnetsInt }, (_, index) => {
                      ...
                      return (
                        <div key={index} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <div style={{ width: 28, height: 28, borderRadius: 6, background: 'rgba(250,189,47,0.1)', border: '1px solid rgba(250,189,47,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontFamily: 'Syne, sans-serif', fontWeight: 700, color: 'var(--gold)', flexShrink: 0 }}>
                            {index + 1}
                          </div>
                          <input
                            className="input-field"
                          ...
                          />
                          {neededMask && (
                            <span className="chip">/{neededMask}</span>
                          )}
                        </div>
                      );
                    })}
                  ...
                  {numSubnetsInt > 0 && hostRequirements.some(h => parseInt(h) > 0) && (
                    <div style={{ marginTop: 10, fontSize: 11, color: 'var(--text-muted)' }}>
                      Total hosts needed: {hostRequirements.reduce((a, h) => a + (parseInt(h) || 0), 0).toLocaleString()}
                    </div>
                  )}
                </div>
              )}
              ...
              <div style={{ display: 'flex', gap: 12, paddingTop: 4 }}>
                <button type="submit" className="btn-primary" style={{ flex: 1, background: 'var(--cyan)', boxShadow: 'none' }}
                ...
                <button type="button" className="btn-secondary" onClick={() => { setBaseIP(''); setOriginalMask(''); setNumSubnets(''); setHostRequirements([]); setError(''); setIpValid(null); }}>
                  Reset
                </button>
              </div>
            </form>
